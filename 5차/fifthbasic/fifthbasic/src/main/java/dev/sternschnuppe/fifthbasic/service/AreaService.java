...
import dev.sternschnuppe.fifthbasic.repository.AreaRepository;
...
public class AreaService {
    ...
    public AreaDto readAreaLL(Double latitude, Double longitude) {
        Optional<AreaEntity> areaEntityOptional = areaRepository.findByLL(latitude, longitude);
        if (areaEntityOptional.isEmpty()) throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        return new AreaDto(areaEntityOptional.get());
    }
}